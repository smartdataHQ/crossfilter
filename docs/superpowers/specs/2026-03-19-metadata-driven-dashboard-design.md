...
14. **Use proven component libraries** — NEVER hand-build standard UI components. Use battle-tested libraries. These are solved problems. Our stack:
    - **Shoelace** (`@shoelace-style/shoelace`) — Primary component library. Web components: `<sl-select>`, `<sl-dropdown>`, `<sl-tooltip>`, `<sl-tab-group>`, `<sl-dialog>`, `<sl-tag>`, `<sl-skeleton>`, etc. Uses Floating UI internally for positioning.
    - **flatpickr** — Date range picker (already installed).
    - **noUiSlider** — Dual-handle range slider.
    - **ECharts** — Charting (already installed).
